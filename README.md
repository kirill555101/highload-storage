...
Всего зарегистрировано 700 млн пользователей, предпложим, что у каждого было по 2 подписки, значит для таблицы user_subscriptions получаем:
* 1 запись = 4 * 3 + 1 * 2 + 8 * 2 = 30 (байт)
* все записи = 2 * 700 * 10e6 * 30 = 39 (Гб)
