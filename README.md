...
Для хранения данных предлагается использовать RAID 10. Это повысит надёжность благодаря полному дубированию данных, а также скорость обработки из-за распараллеливания операций по дискам.

Выделим типы серверов датацентре:
* Nginx
* Backend
* Files
* Redis
...
Предельные значения RPS на ядро для:
* Nginx - 250
* Golang + Gin на 8Гб ОЗУ - 2000

Предельные значения памяти в Тб для:
* Files = 4 652 500
* Redis = 20/1024 = 0.02
* PostgreSQL = 70
...
* Backend = (22 454 * 2) / (2000 * 32) = 1 (шт) => поставим 3 сервера для каждого микросервиса по отдельности
