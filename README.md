...
Среднее число запросов = (3 + 810 + 9 005 + 22 454 + 49) / 2 = 16 161 RPS
...
* Auth microservice - микросвервис для работы с автоирзацией
* Payment microservice - микросервис для работы с оплатой
* Access microservice - микросервис для работы с доступом

Ниже представлена информация про хранение файлов:
* Файлы будут храниться на одном сервере с Access microservice и локальной ext4
* Для дублирования файлов на резервные сервера будет применяться rsync
...
Для хранения данных предлагается использовать RAID 10. Это повысит:
* Надёжность благодаря полному дубированию данных
* Скорость обработки благодаря распараллеливанию операций по дискам
...
* Сервера Backend не будут содержать Access microservice
* Сервера Files будут содержать Access microservice

Конфигурация каждого сервера представлена в таблицах 9-13.

Таблица 9 - Конфигурация сервера для Nginx
CPU | RAM (Гб) | Тип диска | Объем диска (Гб) | Пропускная способность (Гб/с)
--- | -------- | --------- | ---------------- | -----------------------------
16  | 32       | SSD       | 256              | 20

Таблица 10 - Конфигурация сервера для Backend
CPU | RAM (Гб) | Тип диска | Объем диска (Гб) | Пропускная способность (Гб/с)
--- | -------- | --------- | ---------------- | -----------------------------
32  | 256      | SSD       | 256              | 10

Таблица 11 - Конфигурация сервера для Files
CPU | RAM (Гб) | Тип диска | Объем диска (Тб) | Пропускная способность (Гб/с)
--- | -------- | --------- | ---------------- | -----------------------------
32  | 256      | HDD       | 8x8              | 20

Таблица 12 - Конфигурация сервера для Redis
CPU | RAM (Гб) | Тип диска | Объем диска (Гб) | Пропускная способность (Гб/с)
--- | -------- | --------- | ---------------- | -----------------------------
16  | 128      | SSD       | 64               | 10

Таблица 13 - Конфигурация сервера для PostgreSQL
CPU | RAM (Гб) | Тип диска | Объем диска (Тб) | Пропускная способность (Гб/с)
--- | -------- | --------- | ---------------- | -----------------------------
32  | 512      | HDD       | 8x4              | 10
...
Рассчитаем количество серверов по типам в датацентре:
* Nginx = (16 161 * 2) / (250 * 16) = 9 (шт)
* Backend = (16 161 * 2) / (100 * 32) = 11 (шт)
* Files = 4 652 500 / (3 * 64) = 24 232 (шт)
* Redis = 1x2 (1 мастер, 2 реплики)
* PostgreSQL = 128 / (3 * 32) = 1 (шт) => 1x2 (1 мастер, 2 реплики)

Количество серверов в датацентре представлено в таблице 14.

Таблица 14 - Количество серверов в датацентре
