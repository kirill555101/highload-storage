...
На этом уровне располагается Frontend, на котором будет находиться Nginx для передачи статических файлов и балансировки на Backend.

### Уровень приложения
На этом уровне располагается Backend, который будет состоять из 3-м микросервисов:
...
### Уровень данных
На этом уровне располагается Database, на котором будет применяться реализация Master-Slave для репликации и шардирование по id файлов на Master.
